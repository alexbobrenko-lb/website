...
var renderCookieConsent = async () => {
  ...
  const enableEssentials = () => {
    const isSavedConsents = !!getLbCookies(LB_LOCAL_STORAGE_KEY);

    if (
      !isSavedConsents &&
      domain.banner.defaultConsentMode === defaultConsentModes.accept
    ) {
      unblockSources();
      return;
    }
  ...
  /**
   * Applies "Do Not Sell" logic automatically for GPC compliance
   */
  const applyGpcConsent = (domain) => {
    const {
      cookiesAccepted,
      cookiesRejected,
      categoriesAccepted,
  ...
  const fetchDomainInfo = async () => {
    ...
    try {
      ...
      gpcEnabled = localDomain?.banner?.gpcEnabled ?? true;
    } catch (e) {
  ...
  const postCookieConsent = ({
    ...
    categoriesRejected,
    eventType,
    consentBy = consentByTypes.user,
    headers = {},
  }) => {
    if (!domain) return;

    const googleConsentMapper = domain?.googleConsentMapper || [];
    const categoriesById = (domain?.categories || []).reduce(
      (acc, category) => {
    ...
    const linkedCategoriesByGmcType = googleConsentMapper.reduce(
      (acc, item) => {
        if (!item?.googleConsentType) return acc;

        const linkedLightbeamCategories = (item.lbCookieCategories || [])
          .map((linkedCategory) => {
            const categoryFromDomain = categoriesById[linkedCategory?.id];

            return {
              id: linkedCategory?.id || categoryFromDomain?.id || "",
              name: categoryFromDomain?.name || linkedCategory?.name || "",
            };
          })
          .filter((category) => !!category.id || !!category.name);

        acc[item.googleConsentType] = {
          linkedLightbeamCategories,
          linkedLightbeamCategoryIds: linkedLightbeamCategories
    ...
    const getGcmCategoriesPayload = (gtagConsents) => {
      return Object.entries(gtagConsents).reduce(
        (acc, [googleConsentType, status]) => {
          const payload = {
            gmcCategory: googleConsentType,
            linkedLightbeamCategories:
              linkedCategoriesByGmcType[googleConsentType]
                ?.linkedLightbeamCategories || [],
          };

          if (status === "granted") {
            acc.accepted.push(payload);
          } else {
            acc.denied.push(payload);
          }
    ...
    if (
      ...
      if (window.lbCookieConsent?.isGcmOn) {
        const acceptedCategoryIds = new Set(categoriesAccepted || []);
        Object.entries(linkedCategoriesByGmcType).forEach(
          ([googleConsentType, linkedCategoryData]) => {
            ...
            if (isGranted) {
              gtagConsents[googleConsentType] = "granted";
            }
          },
        );
      } else {
      ...
      const gcmCategoriesPayload = getGcmCategoriesPayload(gtagConsents);
      lbCookieConsent.setConsentMode(gtagConsents, {
        eventType,
        acceptedCategories: gcmCategoriesPayload.accepted,
        deniedCategories: gcmCategoriesPayload.denied,
      });
    }
