...
var renderCookieConsent = async () => {
  ...
  const categoryLongToShortId = (longId) => {
    const category = domain?.categories?.find((c) => c.id === longId);
    return category?.shortId ?? longId;
  };

  const categoryShortToLongId = (shortId) => {
    const category = domain?.categories?.find((c) => c.shortId === shortId);
    return category?.id ?? shortId;
  };
  ...
  const savePreferencesInStorage = (categoriesAccepted) => {
    setLbCookies({
      ...
      value: {
        categoriesAccepted: categoriesAccepted.map(categoryLongToShortId),
      },
  ...
  const getPreferencesFromStorage = () => {
    const prefs = getLbCookies(LB_LOCAL_STORAGE_PREFERENCES_KEY);
    if (!prefs) return prefs;
    return {
      ...prefs,
      categoriesAccepted: prefs.categoriesAccepted?.map(categoryShortToLongId),
    };
  };
  ...
  const renderPreferences = async (banner, showPreferencesOnly) => {
    ...
    let savedPreferences = getPreferencesFromStorage();
